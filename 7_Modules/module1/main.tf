...
resource "aws_security_group" "ssg" {
  name        = "module1_security_group" # name should be different on modules
...
resource "aws_instance" "ec2" {
  ...
  user_data                   = <<-EOF
		           ...
		           echo "<h1>!! MODULE-1 !!:  Deployed via Terraform from $(hostname -f)</h1>" | sudo tee /var/www/html/index.html
  EOF
  tags = {
    Name = var.tag
  }
